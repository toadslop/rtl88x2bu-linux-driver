...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)]
pub struct AesCtrNonce(pub [u8; 16]);
...
mod tests {
  ...
  #[test]
  fn aes_key_accepts_valid_lengths() {
    assert!(AesKey::try_from_slice(&[0u8; 16]).is_ok());
    assert!(AesKey::try_from_slice(&[0u8; 24]).is_ok());
    assert!(AesKey::try_from_slice(&[0u8; 32]).is_ok());
  }
