...
extern "C" {
    ...
    #[cfg(config_atmel_rc_patch)]
    fn rtw_rust_expire_auth_sta_skip_atmel(pstapriv: *mut StaPriv, psta: *mut StaInfo) -> U8;
}
...
fn expire_auth_list_kernel(padapter: *mut Adapter) {
    ...
    unsafe {
        ...
        rtw_rust_expire_auth_lock(pstapriv, core::ptr::addr_of_mut!(irql));
        ...
        while rtw_rust_expire_auth_queue_end(phead, plist) == _FALSE {
            ...
            #[cfg(config_atmel_rc_patch)]
            if rtw_rust_expire_auth_sta_skip_atmel(pstapriv, psta) != 0 {
                continue;
            }
        ...
        rtw_rust_expire_auth_unlock(pstapriv, core::ptr::addr_of_mut!(irql));
        for i in 0..flush_num {
            let psta = rtw_get_stainfo_by_offset(pstapriv, flush_list[i] as i32);
            rtw_free_stainfo(padapter, psta);
        }
