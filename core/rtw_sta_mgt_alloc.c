...
#endif /* !CONFIG_RUST || HOST_STA_MGT_TEST || !CONFIG_RUST_STA_MGT_ALLOC */

#if defined(CONFIG_RUST) && !defined(HOST_STA_MGT_TEST)

void rtw_rust_alloc_post_init(struct sta_info *psta, struct sta_priv *pstapriv)
{
	struct recv_reorder_ctrl *preorder_ctrl;
	int i;
	u16 wRxSeqInitialValue = 0xffff;
	...
	for (i = 0; i < 16; i++) {
		...
		_rtw_init_queue(&preorder_ctrl->pending_recvframe_queue);
#if defined(CONFIG_80211N_HT) && defined(CONFIG_RECV_REORDERING_CTRL)
		rtw_init_timer(&(preorder_ctrl->reordering_ctrl_timer), pstapriv->padapter,
			       rtw_reordering_ctrl_timeout_handler, preorder_ctrl);
#endif
		rtw_clear_bit(RTW_RECV_ACK_OR_TIMEOUT, &preorder_ctrl->rec_abba_rsp_ack);
	}
...
void rtw_rust_alloc_lock(struct sta_priv *pstapriv, _irqL *irqL)
{
	_enter_critical_bh(&pstapriv->sta_hash_lock, irqL);
}

void rtw_rust_alloc_unlock(struct sta_priv *pstapriv, _irqL *irqL)
{
	_exit_critical_bh(&pstapriv->sta_hash_lock, irqL);
}

struct sta_info *rtw_rust_alloc_pop_free(struct sta_priv *pstapriv)
{
...
u32 rtw_rust_alloc_hash_attach(struct sta_info *psta, struct sta_priv *pstapriv, const u8 *hwaddr)
{
	s32 index = wifi_mac_hash(hwaddr);

	if (index >= NUM_STA)
		return _FAIL;
	psta->padapter = pstapriv->padapter;
	_rtw_memcpy(psta->cmn.mac_addr, hwaddr, ETH_ALEN);
	rtw_list_insert_tail(&psta->hash_list, &pstapriv->sta_hash[index]);
	pstapriv->asoc_sta_count++;
	return _SUCCESS;
}

void rtw_rust_alloc_mi_update(struct sta_priv *pstapriv)
{
	rtw_mi_update_iface_status(&pstapriv->padapter->mlmepriv, 0);
}
