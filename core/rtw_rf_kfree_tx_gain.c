...
bool rtw_rust_kfree_is_8723d(_adapter *adapter)
{
	return IS_HARDWARE_TYPE_8723D(adapter) ? true : false;
}

u32 rtw_rust_kfree_hal_read_rfreg(_adapter *adapter, u8 path, u32 reg, u32 mask)
{
	return rtw_hal_read_rfreg(adapter, path, reg, mask);
}

void rtw_rust_kfree_log_read_before(_adapter *adapter, u8 path)
{
	if (IS_HARDWARE_TYPE_8723D(adapter)) {
		if (path == PPG_8723D_S1)
			RTW_INFO("kfree gain_offset 0x55:0x%x ",
				rtw_hal_read_rfreg(adapter, RF_PATH_A, 0x55, 0xffffffff));
		else if (path == PPG_8723D_S0)
			RTW_INFO("kfree gain_offset 0x65:0x%x ",
				rtw_hal_read_rfreg(adapter, RF_PATH_A, 0x65, 0xffffffff));
	} else {
		RTW_INFO("kfree gain_offset 0x55:0x%x ",
...
void rtw_rust_kfree_log_read_after(u32 val32)
{
	RTW_INFO(" after :0x%x\n", val32);
}

void rtw_rust_kfree_log_phydm(u8 path)
{
	RTW_INFO("\nkfree by PhyDM on the sw CH. path %d\n", path);
}

void rtw_rust_kfree_chip_write_offset(_adapter *adapter, u8 path, s8 offset)
{
#if !defined(CONFIG_RTL8814A) && !defined(CONFIG_RTL8822B) && !defined(CONFIG_RTL8821C) && !defined(CONFIG_RTL8822C) \
    && !defined(CONFIG_RTL8723F)
	u8 write_value;
#endif
	u8 target_path = IS_HARDWARE_TYPE_8723D(adapter) ? RF_PATH_A : path;
...
u8 rtw_rust_kfree_get_hal_spec_rfpath_num(_adapter *adapter)
{
	return GET_HAL_SPEC(adapter)->rf_reg_path_num;
}
