...
static sint host_aes_decipher(u8 *key, unsigned int hdrlen, u8 *pframe,
			      unsigned int plen)
{
	...
	if ((hdrlen + plen + 8) <= MAX_MSG_SIZE)
		_rtw_memcpy(message, pframe, hdrlen + plen + 8);
	...
	for (i = 0; i < num_blocks; i++) {
		host_bitwise_xor(aes_out, &message[payload_index], chain_buffer);
		payload_index += 16;
		aes128k128d(key, chain_buffer, aes_out);
	}
...
sint host_ccmp_aes_decipher(u8 *key, unsigned int hdrlen, u8 *pframe,
			    unsigned int plen)
{
	return host_aes_decipher(key, hdrlen, pframe, plen);
}
