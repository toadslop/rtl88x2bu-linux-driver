...
/*
 * Host L2 oracle runner for WEP frame encrypt/decrypt (T5 / W3-06).
 */

#if defined(HOST_WEP_FRAME_ORACLE_BUILD) || defined(RUST_SECURITY_ORACLE)
...
#define MAX_NAME 128
#define MAX_BUF HOST_MAX_WEP_FRAME
...
void rtw_wep_encrypt(struct host_adapter *padapter, u8 *pxmitframe);
void rtw_wep_decrypt(struct host_adapter *padapter, u8 *precvframe);

static int parse_fn(const char *obj, size_t obj_len, enum wep_frame_fn *out)
{
	char fn[64];

	if (host_json_parse_string_in(obj, obj_len, "fn", fn, sizeof(fn)))
		return -1;
	if (strcmp(fn, "rtw_wep_encrypt") == 0)
		*out = FN_ENCRYPT;
	else if (strcmp(fn, "rtw_wep_decrypt") == 0)
		*out = FN_DECRYPT;
	else
		return -1;
	return 0;
...
static void setup_adapter(struct host_adapter *adapter, struct vector *v)
{
	memset(adapter, 0, sizeof(*adapter));
	adapter->securitypriv.dot11_privacy_key_index = v->key_index;
	adapter->securitypriv.dot11_def_keylen[v->key_index] = (u32)v->key_len;
	memcpy(adapter->securitypriv.dot11_def_key[v->key_index].skey, v->key,
	       v->key_len);
...
static int run_encrypt_vector(struct vector *v)
{
	...
	rtw_wep_encrypt(&adapter, (u8 *)&xmit);
...
static int run_decrypt_vector(struct vector *v)
{
	...
	plain_len = v->frame_len - v->hdrlen - v->iv_len - 4;
...
int main(int argc, char **argv)
{
	...
	printf("all %zu wep frame vectors passed\n", nvec);
	return 0;
}

#endif /* HOST_WEP_FRAME_ORACLE_BUILD || RUST_SECURITY_ORACLE */
