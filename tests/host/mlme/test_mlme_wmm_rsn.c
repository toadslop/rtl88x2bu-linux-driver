...
static int parse_vector_object(const char *obj, size_t len, void *v)
{
	...
	if (host_json_parse_string_in(obj, len, "expect_out_hex", vec->expect_hex,
...
int main(int argc, char **argv)
{
	...
	if (argc != 2 || host_load_vectors(argv[1], vecs, sizeof(vecs[0]), 8,
					   parse_vector_object, &n))
		return 1;
	for (size_t i = 0; i < n; i++) {
		reset_adapter(&ad, &vecs[i]);
		got = rtw_restruct_wmm_ie(&ad, vecs[i].in, vecs[i].out,
					  (unsigned)vecs[i].in_len,
					  (unsigned)vecs[i].out_off);
		if (got != vecs[i].expect_len ||
		    host_hex_decode(vecs[i].expect_hex, expect, sizeof(expect), &en) ||
