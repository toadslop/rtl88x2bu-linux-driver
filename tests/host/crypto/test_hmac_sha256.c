...
/*
 * Host L2 oracle runner for sha256.c (W2-16a).
 *
 * oracle: core/crypto/sha256.c (+ sha256-internal.c for sha256_vector)
 */
...
static int parse_vector_object(const char *obj, size_t obj_len, void *vec_void)
{
	...
	if (host_json_parse_string_in(obj, obj_len, "fn", fn, sizeof(fn)))
		return -1;
	if (strcmp(fn, "hmac_sha256_vector") != 0)
		return -1;
	if (host_json_parse_bool_in(obj, obj_len, "rust_only", &v->rust_only) != 0)
		v->rust_only = 0;

	if (host_json_parse_string_in(obj, obj_len, "key", hex, sizeof(hex)) == 0) {
		if (*hex &&
		    host_hex_decode(hex, v->key, sizeof(v->key), &key_len))
			return -1;
		v->key_len = key_len;
	}
...
static int run_vector(const struct vector *v)
{
	...
	ret = hmac_sha256_vector(v->key, v->key_len, v->num_elem, addr, v->elem_len, mac);
...
{
	const char *path = "hmac_sha256_vectors.json";
	...
	if (skipped)
		printf("all %zu hmac-sha256 vectors passed (%zu rust-only skipped; "
		       "oracle: core/crypto/sha256.c)\n",
		       executed, skipped);
	else
		printf("all %zu hmac-sha256 vectors passed "
		       "(oracle: core/crypto/sha256.c)\n",
		       executed);
	return 0;
}
