/* SPDX-License-Identifier: GPL-2.0 */
/*
 * CCMP vector fixture types + runner for host L2 tests (W2-09a).
 */
#ifndef HOST_CCMP_VECTOR_H
#define HOST_CCMP_VECTOR_H

#include <stddef.h>

#include "host_types.h"
#include "host_vector_json.h"
#include "host_wifi_types.h"

#define HOST_CCMP_MAX_VECTORS 32

enum host_ccmp_fn {
	HOST_CCMP_FN_ENCRYPT = 0,
	HOST_CCMP_FN_DECRYPT,
	HOST_CCMP_FN_ENCRYPT_PV1,
	HOST_CCMP_FN_256_ENCRYPT,
	HOST_CCMP_FN_256_DECRYPT,
	HOST_CCMP_FN_GET_PN,
};

struct host_ccmp_vector {
	char name[128];
	enum host_ccmp_fn fn;
	enum rtw_amsdu_mode amsdu_mode;
	u8 key[32];
	size_t key_len;
	u8 frame[HOST_VECTOR_MAX_HEX_BUF / 2];
	size_t frame_len;
	u8 hdr[32];
	size_t hdr_len;
	u8 data[HOST_VECTOR_MAX_HEX_BUF / 2];
	size_t data_len;
	size_t hdrlen;
	u8 pn[6];
	int null_pn;
	int keyid;
	u8 a1[6];
	u8 a2[6];
	u8 a3[6];
	int has_a3;
	u8 expected[HOST_VECTOR_MAX_HEX_BUF / 2];
	size_t expected_len;
	/* 1 = function returns non-NULL, 0 = returns NULL */
	int expect_ret;
	/* skipped by C oracle runners; consumed by test_ccmp.c */
	int rust_only;
};

int host_ccmp_parse_vector_object(const char *obj, size_t obj_len, void *vec_void);
int host_ccmp_run_vector(const struct host_ccmp_vector *v);

#endif /* HOST_CCMP_VECTOR_H */
