...
4. **T3** — GitHub Actions (`.github/workflows/host-l2.yml`): L2 host tests on `ubuntu-latest` (`make -C tests/host/domain test` + `make -C tests/host/crypto all` + `make -C tests/host/chplan test` + `make -C tests/host/ie test` + `make -C tests/host/swcrypto test`). L0 on a Rust-kernel container deferred until a cached image exists.
