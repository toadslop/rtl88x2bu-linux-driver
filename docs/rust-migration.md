...
- **C-only / legacy:** `make` and `make KSRC=...` still build when `KDIR` is unset, but migrated crypto TUs (`aes-ctr`, `aes-omac1`, `gcmp`, `aes-siv`, `aes-ccm`) are no longer in `rtk_core` — there is no `ifndef CONFIG_RUST` fallback to the old C objects. Phase 1 targets Rust-enabled kernels (`KDIR` + `LLVM=1`); distro headers without `CONFIG_RUST=y` will not provide those symbols.
