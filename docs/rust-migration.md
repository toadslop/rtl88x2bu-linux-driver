...
- **`.rs` objects** — linked into `88x2bu.ko` only when the target kernel has `CONFIG_RUST=y` (see `rust/kbuild_stub.rs`, `rust/scaffold.rs`, `rust/ffi.rs`, `rust/domain_types.rs`, `rust/aes_ctr.rs`, `rust/aes_omac1.rs`, `rust/gcmp.rs`, `rust/aes_siv.rs`, `rust/aes_ccm.rs`, `rust/sha256_internal.rs`). `rtw_drv_entry` calls `rtw_rust_scaffold_init()` once when Rust is enabled.
