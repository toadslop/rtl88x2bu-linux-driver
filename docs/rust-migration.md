...
Incremental port of this out-of-tree `88x2bu` USB driver from C to Rust-for-Linux (RfL). Work is tracked on [GitHub Issues](https://github.com/toadslop/rtl88x2bu-linux-driver/issues) (authoritative). Draft spec templates and the filing registry live in [`docs/rust-migration/issues/`](rust-migration/issues/README.md).
...
- **`.rs` objects** — linked into `88x2bu.ko` only when the target kernel has `CONFIG_RUST=y` (see `rust/kbuild_stub.rs`, `rust/scaffold.rs`, `rust/ffi.rs`, `rust/domain_types.rs`, `rust/aes_ctr.rs`, `rust/aes_omac1.rs`, `rust/gcmp.rs`, `rust/aes_siv.rs`, `rust/aes_ccm.rs`, `rust/sha256_internal.rs`, `rust/sha256.rs`, `rust/sha256_prf.rs`, `rust/rtw_crypto_wrap.rs`). `rtw_drv_entry` calls `rtw_rust_scaffold_init()` once when Rust is enabled.
- **C-only / legacy:** `make` and `make KSRC=...` still build when `KDIR` is unset, but migrated crypto TUs (`aes-ctr`, `aes-omac1`, `gcmp`, `aes-siv`, `aes-ccm`, `sha256-internal`, `sha256`, `sha256-prf`, `rtw_crypto_wrap`) are no longer in `rtk_core` — there is no `ifndef CONFIG_RUST` fallback to the old C objects. Phase 1 targets Rust-enabled kernels (`KDIR` + `LLVM=1`); distro headers without `CONFIG_RUST=y` will not provide those symbols.
- **Product config** for Phase 1 exit remains default `CONFIG_RTL8822B=y` + `CONFIG_USB_HCI=y` (module name `88x2bu`).

### FFI seam + regenerating bindgen (W1-01 / W1-02)

`rust/ffi.rs` is the mixed-migration ABI edge: it documents which symbols Rust
**owns** vs only **binds**, and re-exports the allowlisted bindgen output from
`rust/bindings/generated.rs`. See the ownership map in that file’s crate docs.

The generated blob is committed; regenerate after changing
`rust/bindings/bindgen_helper.h` or the allowlist in the script:
...
3. Remove `core/crypto/aes-ctr.o` from the `rtk_core` object list (unconditionally — only the Rust `.o` is gated on `CONFIG_RUST`).
...
**C-only builds:** Distro-header builds without `CONFIG_RUST=y` lose the migrated symbols (no `ifndef CONFIG_RUST` fallback to the old C objects). This is intentional for Phase 1 — use a Rust-enabled `KDIR` for migration work.
